...
unsigned long p1start, p1end, p2start, p2end, p3start, p3end;
...
void chkP1()
{
    ...
    const unsigned long td = currentTime - p1start;
...
{
    const unsigned long currentTime = micros();
    const unsigned long delta_time = currentTime - p1start;
    if (digitalRead(phase3pin))
    {
        if (delta_time > 0.25 * holdTime && p3start < p1start)
        {
            digitalWrite(phase3pin, LOW);
            p3end = currentTime;
        }
    }
    else if (delta_time > 1.5 * holdTime)
    {
